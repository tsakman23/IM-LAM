"""In-process, single-run LAPO+BC pipeline runner.
...
prefix with its own 0-based step axis (via the shared ``trainer.log_prefix`` mechanism);
the final eval logs under ``eval/``. Per-stage checkpoints (``${run_id}-N``) and
``--selected_stages`` are preserved. Mirrors experiments/run_slapo.py; the former
subprocess-per-stage entry point (still used by the plain, non-BC experiments/run_lapo.py)
has been removed for this pipeline.
...
Paper (LAPO, the baseline MaskLAM compares against): https://arxiv.org/abs/2312.10812
"""
import os
import sys
...
from eval_lapo import success_eval  # noqa: E402

CONFIG_DIR = os.path.join(os.getcwd(), "experiments", "configs")
CONFIG_MAP = LAPOBCExperiment.config  # {stage_name: default_config_name}
...
def main() -> None:
    ...
    runner = Runner(LAPOBCExperiment)  # reuse the battle-tested arg parser
