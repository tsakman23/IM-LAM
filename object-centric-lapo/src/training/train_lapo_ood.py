...
def train_lapo_slots(
    ...
    if "distractor-low" in base_run_id:
        base_run_id = base_run_id.replace("distractor-low", "distractor-hard")
    if "_seed" in base_run_id:
        base_run_id = base_run_id.replace("_seed", "_ood_seed")
    ...
    load_checkpoint(lapo_checkpoint, model)
    model.to(device)
    model.eval()
    ...
    optimizer = torch.optim.Adam(action_probe.parameters(), lr=cfg.lapo_slots.lr)
    ...
    def compute_loss(model, batch):
        ...
        with torch.no_grad():
            ...
            fdm_loss = model.compute_loss(s_hat, s_future)

        # Compute probe prediction and loss
        action_pred = action_probe(z_t)
        probe_loss, probe_metrics = action_probe_loss_and_metrics(
            action_pred, gt_actions, cfg.task.action_space_type
        )

        metrics = {"fdm_mse": fdm_loss.detach()}
        metrics.update(probe_metrics)

        # Returning probe_loss first ensures scaler.scale(loss).backward() has a valid grad_fn
...
def train_lapo_ood(
    ...
    if cfg.variant == "slots":
        return train_lapo_slots(cfg, videosaur_checkpoint, lapo_checkpoint, slot_selection_path)
    else:
        return train_lapo_masks(cfg, videosaur_checkpoint, lapo_checkpoint, slot_selection_path)
...
if __name__ == "__main__":
    ...
    train_lapo_ood(cfg, args.videosaur_checkpoint, args.lapo_checkpoint, args.slot_selection_path)
