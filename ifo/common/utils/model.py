...
def create_layer_with_args(layer_type: nn.Module, layer_args: Optional[Any]) -> nn.Module:
    """Create a single layer with given layer type and arguments.

    Args:
        layer_type (nn.Module): The type of the layer to be created.
        layer_args (Optional[Any]): The arguments to be passed to the layer.
            Can be a tuple (positional args), dict (keyword args), or None.

    Returns:
        nn.Module: The instantiated layer with the provided arguments.

    Raises:
        ValueError: If layer_type is None or layer_args has invalid type.
    """
...
def miniblock(
    input_size: int,
    output_size: int,
    layer_type: Optional[Type[nn.Module]] = nn.Linear,
    ...
    Args:
        ...
        layer_args (Any, optional): the arguments to be passed to the layer.
            Defaults to None.
        dropout_layer (nn.Module, optional): the type of the dropout layer to be created. Defaults to None.
        dropout_args (Any, optional): the arguments to be passed to the dropout
            layer. Defaults to None.
        norm_layer (nn.Module, optional): the type of the norm layer to be created. Defaults to None.
        norm_args (Any, optional): the arguments to be passed to the norm layer.
            Defaults to None.
        activation (nn.Module, optional): the type of the activation function to be created.
            Defaults to None.
    ...
    if layer_args is None:
        layers: Sequence[nn.Module] = [layer_type(input_size, output_size)]
...
def create_layers(
    layer_type: Optional[Union[nn.Module, Sequence[nn.Module]]], layer_args: Optional[Any], num_layers: int
    ...
    Args:
        layer_type (Optional[Union[nn.Module, Sequence[nn.Module]]]): the type of the layer to be created.
        layer_args (Any, optional): the arguments to be passed to the layer.
        num_layers (int): the number of layers to be created.

    Returns:
        Tuple[Sequence[nn.Module], Any]: a list of layers and a list of args.
...
def per_layer_ortho_init_weights(module: nn.Module, gain: float = 1.0, bias: float = 0.0) -> None:
    """Initialize the weights of a module with orthogonal weights.

    This function recursively applies orthogonal initialization to Linear layers and LSTM layers
    within the provided module. For Sequential and ModuleList containers, it recursively
    initializes each sub-module.

    Args:
        module (nn.Module): Module to initialize.
        gain (float, optional): Gain of the orthogonal initialization. Defaults to 1.0.
        bias (float, optional): Bias value for initialization. Defaults to 0.0.
    """
