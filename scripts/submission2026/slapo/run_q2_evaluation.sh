# Run last stage of slapo pipeline for Q2: Does masking improve downstream performance and sample efficiency?
# Run last stage of slapo pipeline with latent action dim 128 and a sweep of {2k, 4k, 8k, 16k, 32k, 64k, (128k)} GT actions.
# Note, we already have 128k GT actions from Q1 evaluation. We reuse the stage 2 checkpoints for the sweeps.
# Run naming: slapo_q2_<env>_action_count<action_count>_seed<seed>
# Stage 2 checkpoint naming: "./checkpoints/slapo_q1_<env>_seed<seed>-2"
...
for env in "${AVAILABLE_ENVS[@]}"; do
    ...
    for i in "${!ACTION_COUNTS[@]}"; do
        ...
        for seed in "${SEEDS[@]}"; do
            ...
            run_id="slapo_q2_${env//\//_}_action_count${action_count}_seed${seed}"
            python experiments/run_slapo.py \
                ...
                trainer.previous_stage_checkpoint=./checkpoints/slapo_q1_${env//\//_}_seed${seed}-2
        done
    done
