# Run last stage of object-centric lapo slots pipeline for Q2: Does masking improve downstream performance and sample efficiency?
# Run last stage of object-centric lapo slots pipeline with latent action dim 128 and a sweep of {2k, 4k, 8k, 16k, 32k, 64k, (128k)} GT actions.
# Note, we already have 128k GT actions from Q1 evaluation. We reuse the stage 2 checkpoints for the sweeps.
# Run naming: object_centric_lapo_slots_q2_<env>_action_count<action_count>_seed<seed>
# Stage 4 checkpoint naming: "./checkpoints/object_centric_lapo_slots_q1_<env>_seed<seed>-4"
...
for env in "${AVAILABLE_ENVS[@]}"; do
    ...
    for seed in "${SEEDS[@]}"; do
        seed_job_id="object_centric_lapo_slots_q2_${env//\//_}_seed${seed}"
        echo "Submitting single-job action-count sweep for environment: $env with seed: $seed"
        ./scripts/horeka.sh --account hk-project-pai00128 --job_name "cluster_logs/${seed_job_id}" --time 24:00:00 -- bash -lc "
            for action_count in ${ACTION_COUNTS[*]}; do
                echo \"Running evaluation for environment: $env and action count: \${action_count} with seed: $seed\"
                run_id=\"object_centric_lapo_slots_q2_${env//\//_}_action_count\${action_count}_seed${seed}\"
                python scripts/run_pipeline_slots.py \
                    ...
                    --videosaur_checkpoint ./checkpoints/object_centric_lapo_slots_q1_${env//\//_}_seed${seed}-1/videosaur_best.pt \
                    --slot_selection_path ./checkpoints/object_centric_lapo_slots_q1_${env//\//_}_seed${seed}-2/slot_selection.json \
                    --lapo_checkpoint ./checkpoints/object_centric_lapo_slots_q1_${env//\//_}_seed${seed}-3/lapo_slots_latest.pt \
                    --bc_phase_a_checkpoint ./checkpoints/object_centric_lapo_slots_q1_${env//\//_}_seed${seed}-4/bc_phase_a_latest.pt \
