...
from typing import Any, Callable, Dict, Final, Sequence
...
class OriginalWrapper(VectorWrapper):
    ...
    >>> from ifo.common.env.wrapper import OriginalWrapper
    >>>
...
class VecVideoRecorder(RecordVideo, VectorWrapper, gym.utils.RecordConstructorArgs):
    def __init__(
        self,
        env: VectorEnv,
        video_folder: str,
        ...
        """Vectorized wrapper that records videos of rollouts.
        ...
        Args:
            ...
            episode_trigger: Function that accepts an integer and returns ``True`` iff a recording should be started
                at this episode
            step_trigger: Function that accepts an integer and returns ``True`` iff a recording should be started at
                this step
            ...
            fps (int): The frame per second in the video. Provides a custom video fps for environment, if ``None`` then
                the environment metadata ``render_fps`` key is used if it exists, otherwise a default value of 30 is
                used.
            disable_logger (bool): Whether to disable moviepy logger or not, default it is disabled
            log_wandb (bool): Whether to log the video to Weights & Biases or not, on default it is enabled.
        """
        ...
        self.enabled = False  # Prevents errors with WandB logging.
        ...
        if fps is None:
            fps = int(self.metadata.get("render_fps", 30))
...
class ClipAction(gym.ActionWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from ifo.common.env.wrapper import ClipAction
        >>>
...
class OneHotActionWrapper(VectorActionWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from gymnasium.vector import SyncVectorEnv
        >>> from ifo.common.env.wrapper import OneHotActionWrapper
        >>>
...
class MetaWorldImageWrapper(gym.ObservationWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from ifo.common.env.wrapper import MetaWorldImageWrapper
        >>>
    ...
    def observation(self, observation):
        ...
        """
        return {"image": self.render(), "state": observation}

    def render(self) -> np.ndarray:
        """Render the environment and apply the same image post-processing as observations.

        Returns:
            np.ndarray: The rendered image as a uint8 numpy array of shape (height, width, 3).
        """
        ...
        return np.rot90(image, 2) if self._rot_180 else image
...
class NormalizeAction(gym.ActionWrapper):
    ...
    Example:
        >>> import gymnasium as gym
        >>> from ifo.common.env.wrapper import NormalizeAction
        >>>
        >>> # Create an environment
        >>> env = gym.make("Pendulum-v1")
        >>>
