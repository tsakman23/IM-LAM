...
def download_davis_dataset(background_dataset_path: str) -> None:
    ...
    """
    expected_jpg_pattern = os.path.join(
        background_dataset_path, "JPEGImages", "480p", "*", "*.jpg"
    )
    if os.path.exists(background_dataset_path) and glob.glob(expected_jpg_pattern):
        return
...
def make_metaworld_env(
    ...
    **kwargs,
    ...
    if distracting:
        # Keep behavior consistent with distracting DMC: ensure DAVIS is present.
    ...
    if task_name:
        ...
        env_kwargs["vectorization_mode"] = "sync"
...
def make_env(
    ...
    **kwargs,
    ...
    Returns:
        VectorEnv: Wrapped vectorized gym environment.
    """
    if "ALE" in name or "atari" in name:
        game_name = name.split("/")[-1]
        if game_name.startswith(("Frogger", "Frostbite")):
            kwargs.setdefault("screen_size", 128)
            kwargs.setdefault("grayscale_obs", False)
            kwargs.setdefault("sticky_actions", 0.0)
        env = make_atari_env(game_name, num_envs, action_repeat, **kwargs)
    elif "procgen" in name:
        env = make_procgen_env(name, num_envs, action_repeat, **kwargs)
    elif "MiniGrid" in name:
        env = make_minigrid_env(name, num_envs, action_repeat, **kwargs)
    elif "Vizdoom" in name:
        env = make_vizdoom_env(name, num_envs, action_repeat, **kwargs)
    elif "MineRL" in name:
        env = make_minerl_env(name, num_envs, action_repeat, **kwargs)
        ...
        raise NotImplementedError(f"Environment is currently not supported. Name: {name}")
