...
## 1b. SAM-predicted masks (generate + train) - only for a NEW task

The `_sam` dataset ([`tsakman23/visual_masked_distracting_metaworld_sam`](https://huggingface.co/datasets/tsakman23/visual_masked_distracting_metaworld_sam))
adds SAM-**predicted** masks (`pred_mask`, `pred_object_mask`) on top of the ground-truth object-mask
data, for the mask-robustness (GT-vs-SAM) experiments. It is generated by running SAM over the
existing ground-truth frames (`dmw-extract-masks/main.py`), so the GT set must be staged first.

```bash
# 1. stage the ground-truth object-mask data (persistent copy on /data2 - SAM reads it as source)
python scripts/save_local_dataset.py \
  --repo tsakman23/visual_masked_distracting_metaworld --task <task> \
  --splits train test --out-root /data2/masklam/datasets/slapo_local

# 2. generate the SAM masks and push the _sam dataset (run from the extractor dir)
cd dmw-extract-masks
CUDA_VISIBLE_DEVICES=<n> HF_HOME=/data2/masklam/datasets/hf_home python main.py <task> \
  --splits train test \
  --source-root /data2/masklam/datasets/slapo_local \
  --save-local /data2/masklam/datasets/dmw_sam_local \
  --push-to-hub tsakman23/visual_masked_distracting_metaworld_sam
cd ..

# 3. stage the generated _sam set for training (RAM copy, same idea as Section 2)
python scripts/save_local_dataset.py \
  --repo tsakman23/visual_masked_distracting_metaworld_sam --task <task> \
  --splits train test --out-root /tmp/slapo_sam
```

Then train any object-mask model on the SAM masks - e.g. IM-LAM union on SAM data uses the
`imlam_sam_dmw_stage_1` Stage-1 config and logs under group `imlam_union_sam`:

```bash
CUDA_VISIBLE_DEVICES=<n> MUJOCO_GL=egl python experiments/run_slapo.py \
  run_id=im-lam_<task>_union_sam_seed1 env.name=Meta-World/masked-MT1-<task> \
  dataset.dataset_path=/tmp/slapo_sam \
  logger.mode=online logger.group=imlam_union_sam logger.notes="IM-LAM union <task> SAM seed1" \
  trainer.compile=True fabric.precision=bf16-mixed trainer.random_seed=1 \
  --stage stage_1 -cn imlam_sam_dmw_stage_1 \
  --stage stage_2 -cn foreground_masklam_dmw_stage_2 \
  --stage stage_3 -cn foreground_masklam_dmw_stage_3
```
- Stages 2/3 are unchanged (`foreground_masklam_dmw_stage_2/3`); only the Stage-1 config differs from
  the ground-truth run (§4e), selecting the SAM (`pred_*`) masks.
- The other models train on SAM the same way (their own Stage-1 config + `dataset.dataset_path=/tmp/slapo_sam`).
...
### 4g. OCL / LAPO-Slots (baseline)
OCL lives in its **own codebase** (`object-centric-lapo/`), with its own configs and entry point, so it
is run from that directory and takes the staged dataset path via the `DMW_DATASET_PATH` env var (not
`dataset.dataset_path`). One call runs its full VideoSAUR slot pipeline.
```bash
cd /data2/masklam/object-centric-lapo
CUDA_VISIBLE_DEVICES=<n> DMW_DATASET_PATH=/tmp/slapo_local \
  /data2/masklam/conda_env/bin/python scripts/run_pipeline_slots.py \
  --name Meta-World/masked-MT1-<task> --config configs/tasks/dmw_base.yaml \
  --run_id ocl_slots_<task>_seed1 --seed 1 --wandb_group ocl_slots
```
- Stage the object-mask data (`tsakman23/...`) into `/tmp/slapo_local` first (Section 2), same as the
  other object-mask arms, and point `DMW_DATASET_PATH` at that staged copy.
- `<task>` is the same slug used everywhere else (e.g. `push-v3`), so `--name` resolves to
  `Meta-World/masked-MT1-push-v3`. Logs under W&B group `ocl_slots`.
- `configs/tasks/dmw_base.yaml` is the DMW override of the OCL task base. The slot pipeline is the
  heaviest of the models by wall-clock (see the thesis runtime table).
